...
public class LoginActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (isLoggedIn) {
            Intent homePage = new Intent(LoginActivity.this, UserViewActivity.class);
            startActivity(homePage);
            finish();  // Close the login page to prevent going back
        }
    ...
    // Initialize all components for the Login page
    private void initComponents() {
        register = findViewById(R.id.register);
        login = findViewById(R.id.login);
        forgotPass = findViewById(R.id.forgot_password);
    ...
    // Handle all the click events for buttons on the login page
    private void clickListenHandler() {
        ...
        login.setOnClickListener(v -> {
            ...
            CustomerDao customerDao = db.customerDao();
            Customer check = customerDao.findUser(userEmail, userPassword);

            if (check != null) {
                Session.save(LoginActivity.this, "customerID", String.valueOf(check.getCustomerID()));
                Session.save(LoginActivity.this, "isLoggedIn", "true");
                ...
                toast("Login Unsuccessful. Invalid credentials.");
            }
        });

        // Forgot Password Button Click
        forgotPass.setOnClickListener(v -> {
            // Add logic for resetting the password or notifying the user.
            toast("Password Reset Not Implemented.");
        });

        // Customer Button Click (debugging)
        customer.setOnClickListener(v -> {
            CustomerDao customerDao = db.customerDao();
            for (Customer c : customerDao.getAll()) {
                Log.d("LoginActivity", "Customer: " + c.toString());
            }
        });

        // Vehicle Category Button Click (debugging)
        vehicleCategory.setOnClickListener(v -> {
            VehicleCategoryDao vehicleCategoryDao = db.vehicleCategoryDao();
            for (VehicleCategory category : vehicleCategoryDao.getAllCategory()) {
                Log.d("LoginActivity", "Vehicle Category: " + category.toString());
            }
        });

        // Vehicle Button Click (debugging)
        vehicle.setOnClickListener(v -> {
            VehicleDao vehicleDao = db.vehicleDao();
            for (Vehicle vehicle : vehicleDao.getAll()) {
                Log.d("LoginActivity", "Vehicle: " + vehicle.toString());
            }
        });

//         Populate Button Click (Populating database with sample data)
//         Populate Button Click (Populating database with sample data)
        populate.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ...
                // Delete all existing data before inserting new sample data
                vehicleCategoryDao.deleteAll();
                vehicleDao.deleteAll();
                insuranceDao.deleteAll();

                // Create sample vehicle categories and insert into the database
                VehicleCategory vc1 = new VehicleCategory("sedan", 100, -47032, "https://di-uploads-pod12.dealerinspire.com/beavertonhondaredesign/uploads/2017/12/2018-Honda-Accord-Sedan-Sideview.png");
                VehicleCategory vc2 = new VehicleCategory("suv", 101, -13936668, "https://medias.fcacanada.ca//specs/fiat/500X/year-2020/media/images/wheelarizer/2019-fiat-500X-wheelizer-sideview-jelly-WPB_eb45b9d20027fd644f0f273785d919cf-1600x1020.png");
                VehicleCategory vc3 = new VehicleCategory("sports", 102, -4068, "https://images.dealer.com/ddc/vehicles/2019/Lamborghini/Huracan/Coupe/trim_LP5802_b8a819/perspective/side-left/2019_76.png");
                VehicleCategory vc4 = new VehicleCategory("coupe", 103, -3092272, "https://di-uploads-pod12.dealerinspire.com/beavertonhondaredesign/uploads/2017/12/2017-Honda-Accord-Coupe-Sideview.png");
                ...
                // Show toast indicating that the database has been populated
    ...
    // Show toast messages for debugging or alerts
    private void toast(String txt) {
