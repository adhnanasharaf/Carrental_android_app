...
public class BookingCompleteActivity extends AppCompatActivity {
    ...
    private Project_Database database; // Database reference
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Initialize database
        database = Room.databaseBuilder(getApplicationContext(), Project_Database.class, "car_rental_db")
        ...
        // Initialize components and set up UI elements
        initComponents();
        listenHandler();
    ...
    private void initComponents() {
        ...
        // Get the Booking object passed from the previous activity
        booking = (Booking) getIntent().getSerializableExtra("BOOKING");
        chosenInsurance = insuranceDao.findInsurance(booking.getInsuranceID());
        vehicle = vehicleDao.findVehicle(booking.getVehicleID());
    ...
    private long getDayDifference(Calendar start, Calendar end) {
        return ChronoUnit.DAYS.between(start.toInstant(), end.toInstant()) + 2;  // Adds 2 days
    }
    ...
    public void onBackPressed() {
        super.onBackPressed();
        Intent homepage = new Intent(getApplicationContext(), UserViewActivity.class);
        homepage.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP); // Clear the activity history stack
        startActivity(homepage);
    }
}
